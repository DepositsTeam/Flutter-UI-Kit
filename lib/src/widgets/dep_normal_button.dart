...
class DepNormalButton extends StatefulWidget {
  ...
  /// Pass [DepColors] or [Color]. The color for the button's [Material] when a pointer is hovering over it.
  final Color? hoverColor;

  /// Pass [DepColors] or [Color]. The highlight color for the button's [InkWell].
  final Color? highlightColor;

  /// Pass [DepColors] or [Color]. The splash color for the button's [InkWell].
  ...
  /// Button type of [DepButtonShape] i.e, standard, pills, square, shadow, icons
  ...
  /// icon type of [DepPosition] i.e, start, end
...
class DepNormalButtonState extends State<DepNormalButton> {
  ...
  late DepButtonShape shape;
  late double size;
  late DepPosition position;
  ...
  @override
  void didUpdateWidget(DepNormalButton oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    Color getDisabledTextColor() {
      ...
      } else if (widget.type == DepButtonType.outline ||
          widget.type == DepButtonType.outline2x ||
          widget.type == DepButtonType.transparent) {
        return color;
      } else {
        return black;
      }
    ...
    Color getTextColor() {
      if (widget.type == DepButtonType.outline ||
          widget.type == DepButtonType.outline2x ||
          widget.type == DepButtonType.transparent) {
    ...
    } else if (shape == DepButtonShape.square) {
      shapeBorderType = RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(0),
        side: shapeBorder,
      );
    } else if (shape == DepButtonShape.standard) {
      shapeBorderType = RoundedRectangleBorder(
      ...
      if (widget.type != DepButtonType.transparent) {
        if (widget.boxShadow == null && widget.buttonBoxShadow != true) {
          ...
          return BoxDecoration(
            color: widget.type == DepButtonType.transparent ||
                    widget.type == DepButtonType.outline ||
                    widget.type == DepButtonType.outline2x
                ? Colors.transparent
                : color,
            borderRadius: widget.shape == DepButtonShape.pills
                ? BorderRadius.circular(50)
                : widget.shape == DepButtonShape.standard
    ...
    final Widget result = Container(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          customBorder: widget.type == DepButtonType.transparent
              ? null
              : widget.borderShape ?? shapeBorderType,
